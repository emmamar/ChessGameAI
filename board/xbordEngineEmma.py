...
def main():
  ...
  fd = open('/tmp/out', 'w+')
  dictionary_of_functions = {"xboard":xboard, "protover":protover, 
  "accepted":accepted, "rejected":rejected, "new":new, "variant":variant, 
  "quit":quit, "random":random, "force":force, "go":go, 
  "playother":playother, "white":white, "black":black, "level":level,
  "st": st, "sd":sd, "nps":nps, "time":time, "otim":otim, "MOVE":MOVE,
  "usermove":usermove, "?":question, "ping":ping, "draw":draw,
  "result":result, "setboard":setboard, "edit":edit, "hint":hint, "bk":bk,
  "undo":undo, "remove":remove, "hard":hard, "easy": easy, "post":post, 
  "nopost":nopost, "analyze":analyze, "name":name, "rating":rating, 
  "computer":computer, "pause":pause, "resume":resume, "memory":memory, 
  "cores":cores, "egtpath": egtpath, "option": option}
  game = xboardGamePlay()
  while(True):
    try:
...
def given_move(game, given_move):
  game.play_move_human(given_move)
  move = game.play_move_computer()
  move_alg = move.return_algebraic()
  
  with open("/tmp/out", "a") as myfile:
    myfile.write("move " + move_alg +"\n")
    myfile.flush()

  sys.stdout.write("move " + move_alg + "\n")
  sys.stdout.flush()
